...
return [
    'monitor renders SharePoint queue queries and visibility fields' => function (): void {
        ...
        assert(str_contains($monitor, "WHERE status IN ('IMPORT_ERROR', 'RECOVERY_ERROR')"));
        assert(str_contains($monitor, 'Oldest blocking error:'));
    ...
    'monitor queue queries run against queue schema' => function (): void {
        ...
        $oldest = $pdo->query("SELECT file_name, last_error, updated_at FROM sharepoint_file_queue WHERE status IN ('IMPORT_ERROR', 'RECOVERY_ERROR') ORDER BY sharepoint_last_modified_at ASC, id ASC LIMIT 1")->fetch(PDO::FETCH_ASSOC);

        assert($counts[0]['status'] === 'IMPORT_ERROR');
        assert($counts[1]['status'] === 'RECOVERY_ERROR');
        assert($oldest['file_name'] === 'recovery.csv');
    },
];
