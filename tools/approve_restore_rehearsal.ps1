...
function Assert-ProtectedAcl([string] $Path) {
    # Administrators, SYSTEM and explicitly provisioned Apache/scheduler/backup identities are allowed.
    # Broad principals below must never have Write, Modify or FullControl on durable evidence.
    ...
    foreach ($rule in (Get-Acl -LiteralPath $Path).Access) {
        ...
        $danger = $rule.FileSystemRights -band ([Security.AccessControl.FileSystemRights]::Write -bor [Security.AccessControl.FileSystemRights]::Modify -bor [Security.AccessControl.FileSystemRights]::FullControl)
        if ($sid -in $forbidden -and $danger -ne 0) { throw "Trusted backup root ACL grants write access to a broad principal: $sid" }
    }
}
...
function Read-StrictJsonSnapshot([string] $Path, [string] $Label) {
    ...
    try { [D365.RestoreStrictJsonV2]::Validate($text) } catch { throw "$Label failed strict JSON validation: $($_.Exception.Message)" }
    try { $value=$text | ConvertFrom-Json } catch { throw "$Label is not valid JSON." }; if ($null -eq $value) { throw "$Label is not a JSON object." }
...
else { $candidates=@('C:\xampp\backups\d365\uat','C:\xampp\backups\d365\prod','\\100.1.1.166\c$\xampp\backups\d365\uat','\\100.1.1.166\c$\xampp\backups\d365\prod');$trustedRoot=$null;foreach($candidate in $candidates){$root=Get-FullPath $candidate;if($manifestFull.StartsWith($root+'\',[StringComparison]::OrdinalIgnoreCase)){$trustedRoot=$root;break}};if(-not$trustedRoot){throw 'Backup manifest is outside approved D365 backup roots.'};Assert-ProtectedAcl $trustedRoot }
...
if (-not $LocalTestMode) { foreach ($protectedPath in @($trustedRoot,$manifestFull,$auditFull,$evidenceFull,(Split-Path -Parent $outputFull))) { Assert-ProtectedAcl $protectedPath } }
...
$audit=Read-StrictJsonSnapshot $auditFull 'Sanitizer audit';$a=$audit.Value;$auditSource=Assert-InsideRoot ([string](Need $a source_path 'Sanitizer audit')) $trustedRoot;if($auditSource-ine$backup.Path){throw 'Sanitizer audit source path does not match backup.'};$sanitizedPath=Assert-InsideRoot ([string](Need $a sanitized_path 'Sanitizer audit')) $trustedRoot;$sanitized=Read-ArtifactSnapshot $sanitizedPath 'Sanitized backup';Assert-Integer (Need $a sanitized_size_bytes 'Sanitizer audit') 'Sanitized size';if(([string](Need $a source_database 'Sanitizer audit'))-cne$database-or([string](Need $a source_sha256 'Sanitizer audit')).ToLowerInvariant()-cne$backup.Hash-or$sanitized.Hash-cne([string](Need $a sanitized_sha256 'Sanitizer audit')).ToLowerInvariant()-or$sanitized.Size-ne[long]$a.sanitized_size_bytes){throw 'Sanitizer audit does not bind verified artifacts.'}
